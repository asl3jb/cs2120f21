...
ANSWER: first we assume that T is any type and we
  have objects x, y, z, and w of this type. What remains 
  to be shown is that w = z → z = w. To prove this,
  we'll assume the premise that w = z, and in this
  context we need to prove z = w. By the axiom of
  substitutability of equals, and using the fact that
  w = z, we can rewrite w in the goal as z, yielding
  w = w as our new proof goal. And this is already 
